 ...
 */
char* parseRequest(char* request) {// takes in client buff what does it do
  ...
  //FILE * hello = fopen ( " whatsinBuffer.txt", "w");
 // fprintf(hello, "%s", hello);
 // fclose(hello);
  return buffer; // what does it return? lets find out
}

char * request_handle (char * filetype)
{
  
  if(strstr (filetype, ".html") != NULL)
    return "HTTP/1.0 200 OK\r\n"
        "Content-type: text/html; charset=UTF-8\r\n\r\n";
  else if (strstr (filetype, ".gif") != NULL)
    return  "HTTP/1.0 200 OK\r\n"
        "Content-type: image/gif; charset=UTF-8\r\n\r\n";
  else if (strstr (filetype, ".pdf") != NULL)
    return "HTTP/1.0 200 OK\r\n"
        "Content-type: application/pdf; charset=UTF-8\r\n\r\n";
   else if (strstr (filetype, ".png") != NULL)
    return "HTTP/1.0 200 OK\r\n"
        "Content-type: image/png; charset=UTF-8\r\n\r\n";
  else if (strstr (filetype, ".jpeg") != NULL || strstr(filetype, ".jpg") != NULL)
    return "HTTP/1.0 200 OK\r\n"
...
void serve_request(int client_fd){
  ...
  //  FILE *  hello1 = fopen("hello1.txt", "w");
  while(1){//continually take in stuff to that thing
  ...
  // fprintf(hello, client_buf);
  requested_file = parseRequest(client_buf);
  filename[0] = '.';
  strncpy(&filename[1],requested_file,4095);
  //  fprintf(hello1, filename);
  //Check if the file exists
      // if statement 
  struct stat checker;
  // stat(filename,&checker);
  fprintf(stdout , filename);
  
  if ( strstr( filename+1,".")== NULL){
	strcat(filename, "/index.html");
	//	fprintf(stdout, "hello world");
      }
  if(stat(filename, &checker)==0 ){// if this doesnt work use stat(, ,,) or ISREG to check if it exists
    

        char * new_request = request_handle(filename);
	
        send(client_fd,new_request,strlen(new_request),0);
      	// yess file exists 
        read_fd = open(filename,0,0);
	//	char * new_request = request_handle(filename);
	
	fprintf(stdout, new_request);
	
	
        
        //read_fd = open(filename,0,0);//should i change this to new_request? i think so but wont let me
        while(1){
	   ...
	   strcpy(error, error404);
	   strcat(error, error404gif);
	  fprintf(stdout, error);// this comes up so that means other doesnt work and this doesnt work
	  read_fd = open (error, 0, 0);
          int sent = 0;
	  while ( sent < strlen(error)){
	    sent = sent + send(client_fd,error+sent,strlen(error)-sent,0);
	  }
	  }
...
int main(int argc, char** argv) {
	...
	//change directory with change dir
	chdir(*argv);
        //create a thread for this to work on
	 pthread_t newt;
	 pthread_create(&newt, NULL, thread, (void *)sock);
	 pthread_detach(newt);
